...
interface MessageBubbleProps {
  ...
  onReplyToProfile?: (userId: string, username: string, photoURL: string, bio?: string, totalCommunities?: number, commonCommunities?: number, joinedAt?: string) => void;
}
          ...
          >
            <motion.div variants={item} className="flex flex-col items-center text-center gap-6">
              <div className="relative group">
                ...
                <Avatar className={cn(
                  "h-28 w-28 border-8 border-background shadow-2xl transition-all duration-700 group-hover:scale-110 group-hover:rotate-2", 
                  isHidden && "blur-2xl", 
                  isBlurred && "blur-lg"
                )}>
                  <AvatarImage src={isHidden ? undefined : (targetPrivacyData?.photoURL || fallbackPhotoURL)} className="object-cover" />
                  <AvatarFallback className="bg-primary text-white text-4xl font-[900] uppercase">{targetName[0]}</AvatarFallback>
                </Avatar>
                {isHidden && (
              ...
              <div className="space-y-2">
                <h3 className="text-2xl font-black uppercase tracking-tight text-foreground">@{targetName}</h3>
                <div className="flex items-center justify-center gap-3">
                  <Badge variant="secondary" className="bg-primary/10 text-primary border-none text-[9px] font-black uppercase tracking-widest px-3 h-6">
                    <Activity className="h-3 w-3 mr-1.5" /> Synchronized
                  </Badge>
                  {isHidden && (
                    <Badge variant="destructive" className="border-none text-[9px] font-black uppercase tracking-widest px-3 h-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <motion.div variants={item} className="p-5 bg-muted/30 rounded-[2rem] border border-transparent hover:border-primary/20 transition-all group flex flex-col gap-2 relative overflow-hidden">
                <div className="absolute -top-4 -right-4 p-8 opacity-0 group-hover:opacity-10 transition-opacity"><Globe className="h-12 w-12" /></div>
                <div className="flex items-center gap-2 text-primary">
                  <Compass className="h-4 w-4" />
                  <span className="text-[8px] font-black uppercase tracking-[0.2em]">Verse Reach</span>
                </div>
                {isHidden ? <Lock className="h-4 w-4 text-muted-foreground/30" /> : <span className="text-2xl font-black text-foreground">{context?.totalCommunities || 0}</span>}
                <p className="text-[7px] text-muted-foreground font-bold uppercase tracking-widest">Total Communities</p>
              </motion.div>

              <motion.div variants={item} className="p-5 bg-muted/30 rounded-[2rem] border border-transparent hover:border-primary/20 transition-all group flex flex-col gap-2 relative overflow-hidden">
                <div className="absolute -top-4 -right-4 p-8 opacity-0 group-hover:opacity-10 transition-opacity"><Users className="h-12 w-12" /></div>
                <div className="flex items-center gap-2 text-primary">
                  <Waves className="h-4 w-4" />
                  <span className="text-[8px] font-black uppercase tracking-[0.2em]">Mutual Sync</span>
                </div>
                {isHidden ? <Lock className="h-4 w-4 text-muted-foreground/30" /> : <span className="text-2xl font-black text-foreground">{context?.commonCommunities || 0}</span>}
                <p className="text-[7px] text-muted-foreground font-bold uppercase tracking-widest">Shared Universes</p>
              </motion.div>

              <motion.div variants={item} className="p-5 bg-muted/30 rounded-[2rem] border border-transparent hover:border-primary/20 transition-all group flex flex-col gap-2 relative overflow-hidden">
                <div className="absolute -top-4 -right-4 p-8 opacity-0 group-hover:opacity-10 transition-opacity"><Landmark className="h-12 w-12" /></div>
                <div className="flex items-center gap-2 text-primary">
                  <Milestone className="h-4 w-4" />
                  <span className="text-[8px] font-black uppercase tracking-[0.2em]">Verse Age</span>
                </div>
                {isHidden ? <Lock className="h-4 w-4 text-muted-foreground/30" /> : <span className="text-lg font-black text-foreground truncate">{accountAge}</span>}
                <p className="text-[7px] text-muted-foreground font-bold uppercase tracking-widest">Identity Longevity</p>
              </motion.div>

              <motion.div variants={item} className="p-5 bg-primary/10 rounded-[2rem] border border-primary/20 transition-all group flex flex-col gap-2 relative overflow-hidden">
                <div className="flex items-center gap-2 text-primary">
                  <ShieldCheck className="h-4 w-4" />
                  <span className="text-[8px] font-black uppercase tracking-[0.2em]">Security</span>
                </div>
                <span className="text-lg font-black text-primary uppercase">Grade AAA</span>
                <p className="text-[7px] text-primary/60 font-bold uppercase tracking-widest">Protocol Trust</p>
              </motion.div>
            </div>

            <motion.div variants={item} className="p-6 bg-card border rounded-[2.5rem] space-y-4 relative overflow-hidden group">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Zap className="h-4 w-4 text-primary fill-primary" />
                  <span className="text-[9px] font-black uppercase tracking-[0.3em]">Verse Trajectory</span>
                </div>
                <span className="text-[7px] font-black text-muted-foreground/60 uppercase">Live Pulse Analytics</span>
              </div>
              
              <div className="h-20 w-full flex items-end gap-1 px-2 relative">
                {isHidden ? (
                  <div className="absolute inset-0 flex items-center justify-center bg-muted/20 backdrop-blur-sm rounded-2xl">
                    <Ghost className="h-6 w-6 text-primary/20 animate-pulse" />
                  </div>
                ) : (
            ...
            <motion.div variants={item} className="p-6 bg-primary/5 rounded-[2.5rem] border border-primary/10 space-y-3 relative overflow-hidden">
              <div className="absolute top-0 left-0 p-6 opacity-[0.03]"><Activity className="h-16 w-16 text-primary" /></div>
              <div className="flex items-center gap-2">
                <div className="h-4 w-4 rounded-full border border-primary flex items-center justify-center text-primary">
                  <Info className="h-2 w-2" />
                </div>
                <span className="text-[9px] font-black uppercase tracking-[0.3em] text-primary">Intelligence Verification</span>
              </div>
              <p className="text-sm font-medium text-muted-foreground leading-relaxed relative z-10">
                {isHidden 
                  ? "Identity has activated the 'Absolute Anonymity' protocol." 
                  : `Snapshot captured by @${senderName} at sync time. Data integrity verified by Verse node.`}
              </p>
            </motion.div>
          </motion.div>
        </ScrollArea>
