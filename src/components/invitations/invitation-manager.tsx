      ...
      {activeInvite && (
        <Dialog open={!!activeInvite} onOpenChange={() => {}}>
          <DialogContent className={cn(
            "sm:max-w-[450px] w-[92vw] rounded-[3rem] border-none shadow-[0_32px_128px_rgba(0,0,0,0.4)] p-0 overflow-hidden bg-background font-body z-[5000] max-h-[92vh] flex flex-col",
            ...
            >
              <div className={cn(
                "h-28 shrink-0 relative flex items-center justify-center overflow-hidden",
                ...
                <div className="absolute -bottom-8">
                  ...
                  >
                    <Avatar className="h-24 w-24 border-8 border-background shadow-2xl rounded-[2rem] transition-transform group-hover:scale-105">
                      <AvatarImage src={activeInvite.communityIcon} className="object-cover" />
                      <AvatarFallback className="bg-primary text-white text-3xl font-[900]">
                        {activeInvite.communityName?.[0]?.toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                    {isGenesis && (
                      <div className="absolute -top-1 -right-1 p-2 bg-accent rounded-xl shadow-xl border-4 border-background text-white animate-bounce">
                ...
                <div className="absolute top-4 left-6">
                  <div className="bg-white/20 backdrop-blur-md px-3 py-1 rounded-full border border-white/30 flex items-center gap-2 shadow-lg">
              ...
              <ScrollArea className="flex-1 min-h-0">
                <div className="px-10 pt-12 pb-6 text-center space-y-6">
                  <div className="space-y-2">
                    {isGenesis && (
                      <span className="text-[9px] font-black uppercase tracking-[0.4em] text-primary/60 block mb-1">A New Universe Is Born</span>
                    )}
                    <h3 className="text-2xl font-[900] tracking-tighter uppercase leading-tight text-foreground">{activeInvite.communityName}</h3>
                  ...
                  <div className="p-4 bg-primary/5 rounded-[2rem] border border-primary/10 flex items-center gap-4 text-left">
                    ...
                    <div className="flex flex-col">
                      ...
                      <p className="text-[8px] font-bold text-muted-foreground">Original node created by verified entity.</p>
                    </div>
                  </div>
                </div>
              </ScrollArea>

              <DialogFooter className="px-10 pb-6 pt-2 flex flex-col sm:flex-row gap-3 shrink-0">
                ...
                <Button 
                  ...
                  disabled={!!processingId}
                >
                  {processingId ? <Loader2 className="h-4 w-4 animate-spin" /> : <Zap className="h-4 w-4 fill-current" />}
                  Enter Verse
                </Button>
              </DialogFooter>
