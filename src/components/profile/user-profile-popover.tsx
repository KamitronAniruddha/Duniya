    ...
    <div className="relative">
      ...
      <div className="px-5 pb-6 bg-card">
        <div className="relative -mt-10 mb-4 flex items-end justify-between">
          <button 
            onClick={isBlurred ? undefined : onOpenZoom} 
            className={cn(
              ...
              !isBlurred && "hover:scale-105"
            )}
          >
            <Avatar className={cn("h-full w-full rounded-none aspect-square", isBlurred && "blur-xl scale-110")}>
            ...
            {!isBlurred && <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity"><Maximize2 className="h-6 w-6 text-white" /></div>}
          </button>
          ...
          {!isBlurred && (isOnline ? <div className="absolute bottom-1 left-[76px] h-6 w-6 rounded-full border-4 border-card bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.6)]" /> : isIdle ? <div className="absolute bottom-1 left-[76px] h-6 w-6 rounded-full border-4 border-card bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.6)]" /> : null)}
        </div>

        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <h3 className="text-xl font-black tracking-tight">@{cleanUsername}</h3>
            {!isBlurred && (isOnline ? <Badge variant="secondary" className="bg-green-500/10 text-green-600 h-5 uppercase tracking-wider px-2">Online</Badge> : isIdle ? <Badge variant="secondary" className="bg-amber-500/10 text-amber-600 h-5 uppercase tracking-wider px-2">Away</Badge> : null)}
          </div>
          
          <div className={cn("relative", isBlurred && "blur-md select-none pointer-events-none")}>
